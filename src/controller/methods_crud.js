...
import { arrJson } from "../public/json/csvjson.js";
...
methods_crud.crear = (req, res) => {
  ...
  const telefono_rii =
    req.body.telefono_rii != "" ? req.body.telefono_rii : "000";

  const campana_r_1 =
    req.body.campana_r_1 != undefined ? req.body.campana_r_1 : "";
  const campana_r_2 =
    req.body.campana_r_2 != undefined ? req.body.campana_r_2 : "";
  const campana_r_3 =
    req.body.campana_r_3 != undefined ? req.body.campana_r_3 : "";
  const campana_r_4 =
    req.body.campana_r_4 != undefined ? req.body.campana_r_4 : "";
  const campana_r_5 =
    req.body.campana_r_5 != undefined ? req.body.campana_r_5 : "";
  ...
  let createSql =
    ...
    "VALUES" +
    `('${name_e}', ${documento_e}, ${telefono_e}, '${antiguedad_e}', '${campana_e}', '${sede_e}', '${nombre_r}', ${telefono_r}, ${telefono_rii}, '${
      campana_r_1 + campana_r_2 + campana_r_3 + campana_r_4 + campana_r_5
    }', '${
      sede_r_1 +
      sede_r_2 +
      sede_r_3 +
...
methods_crud.actualizar = (req, res) => {
  ...
  conn.query(sqlUptade, (err, resultt) => {
    if (err) {
      console.log(err);
    } else {
...
methods_crud.csvpage = (req, res) => {
  for (let i = 0; i < arrJson.length; i++) {
    let sqlUptadecsv = `UPDATE users SET estado = '${arrJson[i].estado}' WHERE id = ${arrJson[i].id}`;

    conn.query(sqlUptadecsv, function (err, results, fields) {
      if (err) {
        console.log(err);
      } else {
        console.log(i);
